...
import edu.unsw.cse.wiiboard.event.WiiBoardDisconnectionEvent;
...
public class WiiBoardLauncher extends LauncherWrapper implements WiiBoardListener{
	...
	public void wiiBoardDisconnected() {
		WiiBoardServiceDisconnectionEvent se = new WiiBoardServiceDisconnectionEvent();
		Iterator<IListenerCommModule> it = listenersList.iterator();
		while(it.hasNext()){
			IWiiBoardService l = (IWiiBoardService)it.next();
			l.wiiBoardDisconnectionEvent(se);		
		}		
	}
	...
	@Override
	public void wiiBoardDisconnected(WiiBoardDisconnectionEvent disconnectionEvent) {
		WiiBoardServiceDisconnectionEvent se = new WiiBoardServiceDisconnectionEvent();		
		Iterator<IListenerCommModule> it = listenersList.iterator();
		while(it.hasNext()){
			IWiiBoardService l = (IWiiBoardService)it.next();
