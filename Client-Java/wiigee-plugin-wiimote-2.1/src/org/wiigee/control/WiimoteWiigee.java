...
public class WiimoteWiigee extends Wiigee {
    ...
    protected static String pluginversion = "2.0";
    protected static String pluginreleasedate = "03092010";
     ...
     * the one whose MAC address is the one give by parameter. For multiple managed
     * instances of the Wiimote, please use getDevices().
     * 
     * @param The MAC address that the WiiMote has to have to connect
     * @return First visible Wiimote. Null otherwise.
     * @throws java.io.IOException
     */
	public Wiimote getDeviceByMac(String macToMatch) throws IOException {
        this.devices = this.discoverWiimotes(macToMatch);
   ...
   public Wiimote getDevice() throws IOException {
	...
	private Vector<Wiimote> discoverWiimotes(String macToMatch) throws IOException {
        WiimoteDeviceDiscovery deviceDiscovery = new WiimoteDeviceDiscovery(lock);
        LocalDevice localDevice = LocalDevice.getLocalDevice();
        Log.write("Your Computers Bluetooth MAC: " + localDevice.getBluetoothAddress());
        ...
        try {
        	deviceDiscovery.setMacToMatch(macToMatch);
            synchronized (lock) {
                lock.wait();
