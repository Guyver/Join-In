...
import edu.unsw.cse.bluetooth.BluetoothDevice;
...
public class WiiBoardDiscoverer implements DiscoveryListener {
	...
	protected WiiBoardDiscoverer(String boardMac) {
		this.setBoardMac(boardMac);
		isSearching = false;
	 ...
	 */
	synchronized static public WiiBoardDiscoverer getWiiBoardDiscoverer(String boardMac) {
		if (discoverer==null) {
			discoverer = new WiiBoardDiscoverer(boardMac);
		}
	...
	public void deviceDiscovered(RemoteDevice remotedevice, DeviceClass deviceclass) {
		...
		System.out.println("La MAC es "+remotedevice.getBluetoothAddress());
		if (remotedevice.getBluetoothAddress().compareToIgnoreCase(boardMac)!=0){
			System.out.println("It is a WiiBoard but it doesn't have thee MAC address that matches");
			return;
		}
