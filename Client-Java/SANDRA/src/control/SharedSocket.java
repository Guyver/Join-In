...
public class SharedSocket implements Runnable{
	...
	/**
	 * This variable is used to check if it is the first time that we are receiving instructions through the socket.
	 */
	...
	/**
	 * This variable will be used to parse the string which has to be sent through the socket.
	 */
	...
	static public SharedSocket getSharedSocket() {
			...
			//If the shared socket hasn't been created yet...
	        if (sharedSocket == null) {
	            ...
	            //create the actionMap where the Kinect joint values of the devices are going to be stored
	            jointMap=new ConcurrentHashMap<Object, Object>();
	            //create the lastActionMap to store the previous state of the actionMap
	            lastActionMap= new ConcurrentHashMap<Object, Object>();
	            //initialize it with some needed values
	            lastActionMap.put("standStill","true");
	            lastActionMap.put("walk", "false");
	            lastActionMap.put("reached", 0);
	            //create the map tat will contain both the joint and action map.
	...
	public void performTransference(IEventCommModule se){
		//wait until the socketManager is not null
		while (sm == null) {
			try {
		...
		try {
			//This variable will be set to true only when we receive an event that has to be sent 
			boolean validEvent=false;
			//Initialize the string that we are going to send through the socket
			String s ="";			
	...
	public void receiveInstructions(){
		...
		if(receivedMessageConcurrentHashMap!=null){
			...
			if(receivedMessageConcurrentHashMap.containsKey("device")){
				System.out.println("La cadena recibida es: "+receivedMessageConcurrentHashMap);
