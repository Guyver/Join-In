	...
	client.on( 'giveMeUserPictures', function( userKeyParam ) {
		...
		usersModel.findOne({userKey:userKeyParam},function(err, currentUser){
			...
			usersModel.find({team:currentTeam}, function (err, team) {
				...
				for( i in usersReady){
					console.log("I'm iterating over usersReady "+i);
					console.log("The team is: "+usersReady[i].team+"and it maches"+currentTeam);
					if(usersReady[i]!=null && usersReady[i]!=undefined && usersReady[i].team==currentTeam){
					...
					for(i2 in clients){	
						for(i3 in users){
							if(users[i3].ip==javaSocket.remoteAddress){
								...
								console.log("Accepted the user "+i3+" whose key is "+users[i3].userKey+" and the team "+users[i3].team);
								usersReady[userKey] = users[i3];						
							}
						}
						for(i3 in clients){
