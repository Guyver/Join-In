...
public class WiimoteDeviceDiscovery implements DiscoveryListener {
    ...
    public void deviceDiscovered(RemoteDevice newdevice, DeviceClass devclass) {
        ...
        if (!devices.contains(newdevice) &&
            ...
            if(macToMatch.compareTo("")==0){
            	Log.write("No macToMatch has been defined");
            }else{
	            Log.write("Is the wiimote with MAC "+ macToMatch + "? Comparing "+newdevice.getBluetoothAddress()+" with "+macToMatch);
	            if(newdevice.getBluetoothAddress().compareToIgnoreCase(macToMatch)==0){
	            	devices.addElement(newdevice);
	            	Log.write("Yes, it is. Added it to the list of devices");
	            }else{
	            	Log.write("No, it is not. It will not be added to the list of devices");
	            }
