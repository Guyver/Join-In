...
/**	@Name: Test Collision
	...
	@Returns:
*/
Collision_Manager.prototype.testCollision = function( leftHand, rightHand, scene ){
	...
	for ( index in sceneMeshes ){
		...
		else if( sceneMeshes[ index ].name === "Object" || sceneMeshes[ index ].name === "key"){
...
/**	@Name: Update
	...
	@Returns:N/A
*/
Collision_Manager.prototype.update = function( leftHand, rightHand, scene ){

	var info = this.testCollision( leftHand, rightHand, scene );
	
	if( info.flag == "Reset Player" ){
	
		return ( info.mesh.name );
	}
