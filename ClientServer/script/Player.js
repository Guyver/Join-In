...
/** @name CONSTRUCTOR
	...
	@Returns N/A
*/
function Player( name, models, position )
{
	...
	// Hug counter
	this._hugCount = 0;
	// The amount of antiques collected.
	this._antiquesCollected = 0;
	...
	this._rig = new Model( g_jointList, models, this._position );
...
Player.prototype.removeInventory = function( ) 
{
	...
	{
		scene.remove( this._inventory[i] );
		this._inventory.pop();
	}
};
...
Player.prototype.processCommands = function(  ) 
{
	...
	{	
		...
		if(  (this._kinectData[ "handleft" ].y > this._kinectData[ "head" ].y )  
			&& ( this._kinectData[ "handright" ].y < this._kinectData[ "spine" ].y ) )
		{
		
			state = "rotateLeft";
		}
		else if( ( this._kinectData[ "handright" ].y > this._kinectData[ "head" ].y )  
					&& ( this._kinectData[ "handleft" ].y < this._kinectData[ "spine" ].y  ))
		{
		
			state = "rotateRight";
		}		
		else if( ( this._kinectData[ "kneeright" ].y > this._kinectData[ "kneeleft" ].y + 50 )  
					||( this._kinectData[ "kneeleft" ].y > this._kinectData[ "kneeright" ].y + 50 ) )
		{
...
Player.prototype.rotateModelLeft = function( ){
	...
	var torso = this._rig._joint[ "spine"].getPosition();
...
{	
	console.log( "The key from the cookie is : " + this._userKey );
	...
	socket.emit( 'registerMeInServer', { 
		...
		pos 	: 	this._position,
		kinect 	: 	this._kinectData,
		sight 	: 	this._sightNode
	});
